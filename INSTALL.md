...
```bash
cd /opt/earthlink   # after copying this project onto the server
npm install
npm run build
HOST=0.0.0.0 PORT=8080 npm start
```

Open: `http://10.11.12.62:8080` — badge should read **REAL**.
...
```bash
curl -s http://10.11.12.62:8080/api/traffic/health
curl -s http://10.11.12.62:8080/api/traffic | jq '{in:.inboundCount,out:.outboundCount,active:.activeCount}'
```
